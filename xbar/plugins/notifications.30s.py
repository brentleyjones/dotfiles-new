...
# GitHub.com
github_api_key = os.getenv( 'GITHUB_TOKEN', '__REPLACE_GITHUB_TOKEN__' )

# GitHub:Enterprise (optional)
enterprise_api_key = os.getenv( 'GITHUB_ENTERPRISE_TOKEN', '__REPLACE_GITHUB_ENTERPRISE_TOKEN__' )
enterprise_api_url = os.getenv( 'GITHUB_ENTERPRISE_API', '__REPLACE_GITHUB_ENTERPRISE_API__' )

active = '#4078C0'
inactive = '#7d7d7d'
important = '#ff0000'

last_checked_path = '/private/var/tmp/github_notifications_last_checked'
...
def is_important( notification ):
    ...
    if reason == 'author':
        return True
    elif reason == 'invitation':
        return True
    elif reason == 'mention':
        return True
    elif reason == 'team_mention':
        return True
...
def notify(title, text):
    os.system("""
              osascript -e 'display notification "{}" with title "{}" sound name "Breeze"'
              """.format(text, title))
...
def make_github_request( url, method='GET', data=None, enterprise = False ):
    try:
        ...
        return json.load( response ) if int(response.headers.get('content-length', 0)) > 0 else {}
...
def print_notifications( notifications, last_checked, enterprise=False ):
    notifications = sorted( notifications, key=lambda notification: (notification['repository']['full_name'], not is_important(notification)) )
    for repo, repo_notifications in groupby( notifications, key=lambda notification: notification['repository']['full_name'] ):
        if repo:
            ...
            for notification in repo_notifications:
                important = is_important(notification)
                formatted_notification = format_notification( notification )
                print_bitbar_line(
                    refresh='true',
                    font='HelveticaNeue-Medium' if important else None,
                    **get_dict_subset( formatted_notification, 'title', 'href', 'image', 'templateImage' )
                )
                print_bitbar_line(
                    refresh='true',
                    title='%s - Unsubscribe' % formatted_notification['title'],
                    alternate='true',
                    bash=__file__,
                    terminal='false',
                    param1='unsubscribe',
                    param2=formatted_notification['thread'],
                    param3=notification['subscription_url'],
                    param4='--enterprise' if enterprise else None,
                    **get_dict_subset( formatted_notification, 'image', 'templateImage' )
                )
                if important:
                    updated = dateutil.parser.parse(notification['updated_at'])
                    if updated >= last_checked:
                        notify( title="GitHub Notification", text=formatted_notification['title'] )
...
if len(sys.argv) > 1:
    ...
    if command == 'readrepo':
        ...
        print('Unsubscribing from %s' % thread_url)
        make_github_request( url=subscription_url, method='PUT', data={"ignored": True}, enterprise=enterprise )
        make_github_request( url=thread_url, method='PATCH', data={}, enterprise=enterprise )

else:
    last_checked = datetime.min
    if os.path.exists(last_checked_path):
        with open(last_checked_path, 'r') as file:
            last_checked = dateutil.parser.parse(file.readline())
    ...
    has_important_notifications = any(is_important(n) for n in github_notifications + enterprise_notifications)
    color = important if has_important_notifications else active if has_notifications else inactive
    ...
    if is_github_defined:
        if len( github_notifications ):
            ...
            print_notifications( github_notifications, last_checked )
        else:
            print_bitbar_line(
                title='GitHub \u2014 No new notifications',
                color=inactive,
                href='https://github.com/notifications',
            )
    ...
    if is_github_enterprise_defined:
        if len( enterprise_notifications ):
            if is_github_defined:
                print('---')
            print_bitbar_line(
                title=( 'GitHub:Enterprise \u2014 %s' % plural( 'notification', len( enterprise_notifications ) ) ),
                color=active,
                href='%s/notifications' % re.sub( '/api/v3', '',  enterprise_api_url ),
            )
            print_notifications( enterprise_notifications, last_checked, enterprise=True )
        else:
            print('---')
            print_bitbar_line(
                title='GitHub:Enterprise \u2014 No new notifications',
                color=inactive,
            )
